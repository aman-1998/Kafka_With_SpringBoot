...
public class BadFeedbackConsumer {
	
	@KafkaListener(id = "myListener1", topics = "${test.topic.bad.feedback}", groupId = "${test.group.feedback}", 
	...
	public void consume(Feedback message, @Header(name = KafkaHeaders.RECEIVED_TOPIC, required = false) String topic, 
		...
		System.out.println("====> Message received by BadFeedbackConsumer: " + message);
		...
		try {
			...
			System.out.println("An exception occurred in BadFeedbackConsumer : " + ex.getMessage());
			throw new RuntimeException("An exception occurred in BadFeedbackConsumer : " + ex.getMessage());
		}
	}
}
