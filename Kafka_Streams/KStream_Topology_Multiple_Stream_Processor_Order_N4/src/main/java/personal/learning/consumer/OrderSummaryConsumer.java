...
public class OrderSummaryConsumer {
	...
	@KafkaListener(id = "myListener1", topics = "${test.topic.order.summary}", groupId = "${test.group.order}", 
			   	   autoStartup = "true", containerFactory = "kafkaListenerContainerFactory")
	public void consume(OrderSummary message, @Header(name = KafkaHeaders.RECEIVED_TOPIC, required = false) String topic, 
										  @Header(name = KafkaHeaders.RECEIVED_PARTITION, required = false) int partition,
										  @Header(name = KafkaHeaders.OFFSET, required = false) long offset) 
										  throws Exception {
										  // @Headers Map<String, Object> header) {
		System.out.println("====> Message received by OrderSummaryConsumer: " + message);
		...
		try {
			...
			System.out.println("An exception occurred in OrderSummaryConsumer : " + ex.getMessage());
