...
public class OverallBadWordCountConsumer {
	
	@KafkaListener(id = "myListener5", topics = "${test.topic.overall.bad.word.count}", groupId = "${test.group.feedback}", 
	...
	public void consume(Long message, @Header(name = KafkaHeaders.RECEIVED_TOPIC, required = false) String topic, 
		...
		System.out.println("====> Message received by OverallBadWordCountConsumer: key="+ key + ", value=" + message.toString());
		...
		try {
			...
			System.out.println("An exception occurred in OverallBadWordCountConsumer : " + ex.getMessage());
			throw new RuntimeException("An exception occurred in OverallBadWordCountConsumer : " + ex.getMessage());
		}
	}
}
