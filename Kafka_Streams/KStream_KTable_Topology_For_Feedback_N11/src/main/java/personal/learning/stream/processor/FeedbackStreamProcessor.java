...
public class FeedbackStreamProcessor {
	...
	public void processFeedback(StreamsBuilder builder) {
		...
		// Same logic as KStream_KTable_Topology_For_Feedback_N10
		// Just different way of doing same thing.
		
		
		sourceStream.flatMap(FeedbackUtil.convertToWords()).split().branch(FeedbackUtil.isGood(), 
				Branched.<String, String>withConsumer(ks -> {
						ks.repartition(Repartitioned.as(positiveWordTopic))
						  .groupByKey().count().toStream().to(positiveWordCountTopic, Produced.with(Serdes.String(), Serdes.Long()));
					
						ks.groupBy((key, goodWord) -> goodWord).count().toStream().to(overallGoodWordCountTopic, 
							  													  Produced.with(Serdes.String(), Serdes.Long()));
		})).branch(FeedbackUtil.isBad(), 
				Branched.<String, String>withConsumer(ks -> {
					ks.repartition(Repartitioned.as(negativeWordTopic))
					  .groupByKey().count().toStream().to(negativeWordCountTopic, Produced.with(Serdes.String(), Serdes.Long()));
