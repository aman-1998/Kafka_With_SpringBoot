...
public class MessagePublisher3 {
	...
	public void sendMessageEpsilon(String partition, Customer message) {
		
		CompletableFuture<SendResult<String, Object>> future = kafkaTemplate.send(topicEpsilon,
	...
	public void sendMessageZeta(String partition, Customer message) {
		
		CompletableFuture<SendResult<String, Object>> future = kafkaTemplate.send(topicZeta,
															   Integer.valueOf(partition), 
															   null, message);
		future.whenComplete((result, ex) -> {
			if(ex == null) {
